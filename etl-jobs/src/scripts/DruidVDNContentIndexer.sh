...
# Configurations
druidCoordinatorIP=""
dataSourceName="vdn-content-model-snapshot"
today=`date +%Y-%m-%d`
interval="2019-01-01_$today"
now=`date +%Y-%m-%d-%s`
home=`echo $HOME`
ingestionSpecFilePath="${home}/etl-jobs-1.0/druid_models/vdn_content_index_batch.json"
jobJarPath="${home}/etl-jobs-1.0/etl-jobs-1.0.jar"
jobConfPath="${home}/etl-jobs-1.0/resources/ESCloudUploaderVDN.conf"
...
--class org.ekstep.analytics.jobs.ESCloudUploaderVDN \
${jobJarPath}
