...
vdnDataSourceName="vdn-content-model-snapshot"
today=`date +%Y-%m-%d`
interval="2019-01-01_$today"
now=`date +%Y-%m-%d-%s`
home=`echo $HOME`
ingestionSpecFilePath="${home}/etl-jobs-1.0/druid_models/content_index_batch.json"
vdnIngestionSpecFilePath="${home}/etl-jobs-1.0/druid_models/vdn_content_index_batch.json"
...
# delete older segments
curl -X 'DELETE' -H 'Content-Type:application/json' http://${druidCoordinatorIP}:8081/druid/coordinator/v1/datasources/${vdnDataSourceName}/intervals/${interval}
